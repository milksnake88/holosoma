...
def load_motion_data(
    ...
    if task_type == "robot_only":
        if data_format in ("lafan", "sfu", "bones"):
            # All BVH-based: use data_utils/extract_global_positions.py to get the .npy files
            ...
            if data_format == "lafan":
            ...
            elif data_format in ("sfu", "bones"):
                # Both are y-up and right-handed, so rotate (x, y, z) -> (x, -z, y) instead of
                ...
                # SFU and bones sequences are captured at 120 fps: downsample to 30 fps
